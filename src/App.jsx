...
function buildWaLink(product, agreedPrice) {
  const msg =
    ...
    "\nPrix proposé en discussion: " + fmt(product.price) +
...
function NegotiationChat({ product, onClose }) {
  const [messages, setMessages] = useState([
    {
      from: "bot",
      text:
        'Bonjour ! Vous négociez pour "' + product.name + '", affiché à ' +
        fmt(product.price) +
  ...
  const handleOffer = (offerRaw) => {
    ...
    if (offer >= price) {
      ...
      push("bot", "Parfait, " + fmt(price) + " c'est notre prix affiché — marché conclu ! Cliquez ci-dessous pour confirmer sur WhatsApp.");
      return;
    }
    ...
    if (offer < minPrice) {
      ...
      setRoundCount(round);
      if (round >= 3) {
        push("bot", "Je comprends, mais je ne peux vraiment pas descendre plus bas que " + fmt(minPrice) + " pour cet article. C'est ma dernière offre — vous voulez conclure à ce prix ?");
      } else {
        push("bot", "Ce prix est un peu trop bas pour moi. Je peux descendre jusqu'à " + fmt(minPrice) + " maximum. Voulez-vous proposer un montant entre " + fmt(minPrice) + " et " + fmt(price) + " ?");
      }
      return;
    }
    ...
    if (offer >= minPrice * 1.04 || roundCount >= 1) {
      ...
      push("bot", "Marché conclu à " + fmt(offer) + " ! C'est un bon prix. Cliquez ci-dessous pour confirmer la commande sur WhatsApp.");
    } else {
      setRoundCount((r) => r + 1);
      push("bot", "Je vous propose plutôt " + fmt(mid) + ". C'est un bon compromis. Ça vous convient ?");
    }
  };
        ...
        {!closed ? (
          ...
          <div className="negchat-confirm">
            <div className="negchat-confirm-price">
              <Check size={16} />
              <span>Prix convenu : <strong>{fmt(agreedPrice)}</strong></span>
            </div>
...
export default function App() {
  ...
  const error = productsHook.error;
